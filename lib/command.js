...
function stringify() { /* private dangler */
    ...
    let logCode = `var log = makeLogger('test');`
    ...
    let closeLogCode = `log.close();`

    const inc = this.includes.join(';');
    const main = err_open + logCode + res_get + this.code + '.apply(this' + args + ');\n' + err_close + res_write + closeLogCode;
