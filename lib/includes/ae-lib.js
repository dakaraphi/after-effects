...
function copyKeyFramesFromLayerToLayer(sourceLayer, targetLayer, keyFramesOfProperty, adjustKeyframeFn) {
    const pointProperties = findProperties([sourceLayer], [{name: keyFramesOfProperty}]);
    const targetPoints = findProperties([targetLayer], [{name: keyFramesOfProperty}])

    pointProperties.forEach(pointProperty => {
        forEachKeyFrame(pointProperty, keyframe => {
            // need to copy to the same point control by same name
            const targetPoint = targetPoints.find(point => point.parentProperty.name === pointProperty.parentProperty.name)
