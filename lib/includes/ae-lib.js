...
function cloneAsSafeValue(object, depth) {
...
function makeLogger(name) {
    const metadataLocation = '~/.ae_node_script'
    const scriptFolder = new Folder(metadataLocation)
    scriptFolder.create()
    const logFile = new File(metadataLocation + '/after_effects-script.log');
    logFile.open("w");  // open for writing and replace contents
    logFile.encoding = "UTF-8";

    function expandObject(object) {
        if (object instanceof Object) {
            return JSON.stringify(cloneAsSafeValue(object))
        }
        return object
    }
...
function propertySettingsAsJSON(layer) {
    ...
    forEachProperty(layer, (property, parentProperty) => {
        let value = null;
        try {value = property.value} catch(e) {}
...
function findAllProperties(composite, filters) {
    return findProperties(layers(composite), filters);
}
...
function makeLayersInterface(composite) {
    ...
    function addAll(assets, onAddFn) {
