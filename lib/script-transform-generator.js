...
function makeSource(afterEffectsFunctionArguments, code, includes, result_file) { /* private dangler */
    let args = '';
    ...
    const source = `
            ...
            (function(result){
                var file = File(Folder.temp.absoluteURI+"/${result_file}");
...
function makeTransformGenerator(originalFunction, functionParameters, options, resultFile) {

    //Babelify function as an expression otherwise it breaks when isolated
    const es3_transformedFunction = babelify(`(${originalFunction.toString()})`);
    ...
    return {
        ...
        code,
        includes,
        makeSource: () => makeSource(functionParameters, code, includes, resultFile)
    }
}
